...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    if (header_len < 0 || (size_t)header_len >= sizeof(header) - 1) return -1;
    header[header_len++] = '\0';
    ...
    if (len > 0 && data) memcpy(full + header_len, data, len);
    ...
    char shard_dir[512];
    snprintf(shard_dir, sizeof(shard_dir), "%.2s", final_path);
    ...
    char tmp_path[640];
    snprintf(tmp_path, sizeof(tmp_path), "%s/tmp_obj_%d", shard_dir, getpid());
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    if (payload_offset > file_size || data_len != file_size - payload_offset) {
        free(buf);
        return -1;
    }
    ...
    if (data_len > 0) {
        memcpy(out, buf + payload_offset, data_len);
    }
    out[data_len] = '\0';
